...
import com.dark_phoenix09.app1pcon2k20.databinding.FragmentQuizGameBinding
...
class QuizGameFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
       ...
       // sharedPreferences = activity?.getSharedPreferences("SP_HIGH_SCORE", Context.MODE_PRIVATE)!!
    ...
    private fun getScore(){
        if(score>=2){
            val action = QuizGameFragmentDirections.actionQuizGameFragmentToQuizWonFragment(score,wrongAnswerList.toTypedArray())
            requireView().findNavController().navigate(action)

        }
        else{
            val action = QuizGameFragmentDirections.actionQuizGameFragmentToQuizLostFragment(score,wrongAnswerList.toTypedArray())
            requireView().findNavController().navigate(action)
        }
    }
